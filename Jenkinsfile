			...
			steps{
				script{
					...
					withCredentials([usernamePassword(credentialsId: 'neo4jUsernamePassword', passwordVariable: 'pass', usernameVariable: 'user')]){
	                    sh "java -Dlogback.configurationFile=src/main/resources/logback.xml -jar target/fireworks.jar --user $user --password $pass --folder ./config --output ./${env.OUTPUT_FOLDER}"
